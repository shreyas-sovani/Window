...
- **Claim session** — scan Finalized Windows across **every venue** (no `venueId` scoping — winnings must not hide behind the live Window's venue), dedupe by `marketId`, apply ClaimPlan (void both sides, winner only, skip zero), then redeem. Newest-expired first; capped at the 40 most recently finalized Windows — the empty-scan copy names that bound. Preview is unique Windows plus expected collateral (`readClaimSession`: winner fee-adjusted per Window via `SettledWindow.feeBps` — the adapter fetches fees only for held Windows; void at half), not raw redeem intents. Copy names Windows and tUSDC (`claimSessionCopy` / `claimReceiptCopy`). A failed redeem does not abort the rest (`executeClaims` continues per Window; `failed` is how many did not finish). All-fail rethrows so RevertCopy can speak.
- **Window board** — the read model for the live series: live Window, implied Up, Call plans, WalletGate, thin-book and short-collateral flags. Prefers a **Stake quote** when the live book can fill the stake; otherwise sizes from the top of book. With **no book at all, the plan is `bad-price`** — a Call is never sized at an invented 50%. Wallet chrome and the Call ticket render it; they do not re-derive it.
- **Stake quote** — live-book size of a collateral stake: raw quantity, protective limit in the traded outcome's own terms, and escrow (max loss). Missing or below-lot quotes fall back to a single-ask Call plan so a cold watch does not disable Call — but a missing book does.
...
- **Rest quote** — post-only bid sized like a Call but priced at the bid (`restLimit`: Up = bid, Down = 1 − ask). Lives in the Book drawer. Default Call stays IOC. A crossed book is `would-take`; a missing bid is `no-rest`. Escrow stays until fill or cancel (Open ticket). A Rest **expires when its Window locks** — the pool enforces `expireTimestampNs ≤ market expiry` and defaults to it, so a Rest cannot outlive the market.
- **Series P&L** — Wallet P&L filtered to the selected series (asset + cadence, snapped). Lives on the history strip. **Tape-only:** fills and open marks; Claim redeem payouts are not fills and are not counted — the label says so. Not the Series record (that is Up/Down/Void tally, not money).
...
- **Liquidity preview** — `fillEstimate` walks the visible top-5 book levels (Up through YES asks cheapest-first; Down through NO asks = 1 − YES bids, best first) for the requested stake: expected filled stake, average execution odds, unfilled remainder, and the whole-walked-depth ceiling behind "Use max". An **estimate from displayed depth, never a promise** — the Call still sends its own protective limit, and copy always says "est.". Null when that side has nothing executable.
- **Proof card** — plain-text receipt of a Call this terminal **witnessed** (`CallReceipt`: decision, Line, lock time, stake → contracts @ avg odds, if-right/at-risk, explorer tx). `settledProofCard` appends the result (joined from series history by `marketId`) and the oracle receipt when the indexer has the question id. Clipboard or Web Share, no backend, no fake social metrics. Not position archaeology — only Calls this session recorded.
- **Onboarding step** — the one guided next action, from `nextStep`: connect → switch → gas (only when the wallet provably has no STT — unknown never blocks) → mint (collateral below stake) → approve (names the exact stake) → wait (Window not callable, with its reason) → call. Claim is deliberately **not** in the chain — it is a rewards section beside trading, never a gate before the next Call. The panel shows title + explanation + one prominent action; Call buttons stay visible but disabled with their reason. Not the WalletGate (that stays the write-gate); this is the presentation layer's read of the same truth.
- **Chip status** — `chipStatus` per series: `trading` (a live Trading row), `waiting` (locked/rolling), `none`. The cadence pills mark waiting, and the selected pair wears an `auto` badge when `autoSeries` chose it.
- **Roll** — repeat the last witnessed Call on the successor Window of the same series: same asset, cadence, side, and stake, one press (`rollPrompt` + `LastCall`). The wallet still signs; nothing repeats on its own. The cadence pill with the best live headroom wears a `best` badge (`hottestCadence`). Not an agent — no session key, no automation (see SDK-FEEDBACK #9 for why a hot-key roll bot is not shippable on this SDK).
- **Market health** — one grade per live Window from spread, walked executable depth, and time-to-lock (`marketHealth`): `strong` / `fair` / `thin` / `none`. Two-sided top with a cold depth watch grades on spread alone and says "top of book" — never claims depth it cannot see. "Thin + locks in" starts at the same headroom where Calls actually close.
