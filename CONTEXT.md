...
**Window** is a consumer Up/Down terminal for dreamDEX Event Contracts on Somnia. The homepage is one live price window, not a CLOB blotter.
...
- **Oracle receipt** — public resolution trail at `https://prd.oracle.somnia.host/questions/{oracleQuestionId}?view=graph`.
...
- **Claim session** — scan Finalized Windows, apply ClaimPlan (void both sides, winner only, skip zero), then redeem. Newest-expired first; cap the scan so a long tail cannot hide recent winnings.
...
- **Series history** — recently Finalized Windows for the same asset and cadence, including the Line and Up/Down/Void result.
...
- **Cadence** — series length (60s / 5m / 15m / 1h / 4h / 24h). Indexer `intervalSec` is derived from expiry − start and can be a few seconds off; snap to the canonical cadence before matching. Display with `cadenceLabel` (never `intervalSec / 60` + `"m"`, which prints 24h as 1440m).
...
- **Claim primary** — when the Claim session has at least one redeem intent, the tote's primary chrome is Claim (`totePrimary`), even if the successor Window is already Trading. Connect and switch still precede Claim. tUSDC approve is not required to redeem. Call Up/Down stay available as secondary when the successor is callable.
...
- **Explorer proof** — Shannon explorer URL for a write (`explorerTx`) or the connected address (`explorerAddress`). Call session returns the IOC hash; the toast links it. WalletBar and the P&L tape already link address and fills. Not the Oracle receipt.
