...
| W-117 | Demo depth is a real book: `demoDepthFor` six-level two-sided ladder (1-pt spread, ≈2.7k tUSDC executable a side) feeds the odds hook, market health, fill estimate, and a ladder-walking `quoteStake`; per-cadence `demoWindowSec` keeps every chip callable for two thirds of its Window; cadence-salted pools; growing anonymous pool tape + moving price feed; account-scoped demo claims; holdings credited on fill; `demoExchangeFor` session singleton | exchange (demo) / ui | P0 | done | yes |
| W-119 | Accept-path FOK diagnosis: `revertCopy` maps `FillOrKillNotFillable` ("the opposite side cannot fill the whole stake") and `ERC20InsufficientAllowance`, and names any other pool error instead of blaming gas/Shannon; new `acceptDepthGate` disables the accept — before any transaction — when the depth read shows the end of the opposite ladder and it cannot cover the stake (fails open on a truncated or cold read) | Duel / RevertCopy / ui | P0 | done | yes |
| W-118 | Demo links resolve end to end: `demoDecorateHref` no longer drops the route and `d=` hint (every demo challenge link was dead); `bootstrapSearchLink` lifts a fragment-stripped link into `#/app?…`; addressed links let a disconnected recipient connect; the accept floor gates on quoted escrow with a cent of prefill headroom (quantization could refuse a valid accept after the FOK filled); a published accept renders verifying until the tape reads it back; invite countdown on the strip; demo-only simulated-opponent accept records the whole loop in one browser | ui / Duel / exchange (demo) | P0 | done | yes |

PRD: docs/PRD.md. Glossary: CONTEXT.md.
