...
**0:10 — The read model.** Point at Line / the lock ring / Implied Up / the Book cell. "Odds come from the SDK's live order book over WebSocket, with a polled top-of-book fallback. The ring is the Window's remaining time — it depletes and turns red as it locks. The Book cell grades spread and executable depth honestly: cold depth says 'top of book', no book says no Call." Hover the book drawer: "Real depth, human units, spread in the middle."

**0:25 — The Call.** Tap preset **10**. "Sizing runs through a live stake quote — 10 tUSDC becomes this many contracts at the protective limit. IOC only: it takes what's there and cancels the rest. Nothing rests." Press **Call Up**. Sign. Point at the toast, then the position banner with the settle preview: "If Up wins, if Down wins, if Void — priced before I commit, venue-fee aware." Mention the roll bar if it appears after the Window rolls: "One press repeats the same Call on the successor — the wallet still signs."
...
**1:15 — Close.** "Zero custom contracts. Every trade is the dreamDEX SDK on Somnia Shannon. Domain layer is pure TypeScript, 219 tests including a full-UI integration run against a deterministic fake exchange, adapters swappable. docs/SDK-FEEDBACK.md is our nine-item report back to the dreamDEX team."
...
- Book empty: the ticket says odds are not tradable and the Call buttons stay off with their reason — Window never invents a price. Switch cadence or wait for a maker; say so, it's by design.
