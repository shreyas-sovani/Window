...
8. **Unified `createOrder` cannot set order expiry.** Domain `expireTimestampNs()` exists (gotcha: `0` reverts). `CreateOrderParams` only has `timeInForce` / `postOnly` / slippage. IOC leftovers do not rest, so Window does not need GTC expiry on the default path. Rest quotes use `postOnly: true` and still cannot age off through the unified helper.
