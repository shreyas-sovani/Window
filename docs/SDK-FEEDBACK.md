...
9. **The operator / session-key model is not reachable for Event Contracts through the SDK.** The pool supports it — `tradeAbi` ships `placeBinaryOrderFor(address owner, …)` ("settlement-extraction v2") — but `Trader` exposes no binary placement-for path: `PlaceOrderParams` has no `owner`, and `setOperatorApprovalForPool` is documented as "ONE **SpotPool**". The Bot Kit's session-key flow (docs/session-keys.md, `operator-setup.ts`) is likewise spot-only — `Pool.place`/`Pool.cancel` in `@dreamdex-bot-kit/core` — while `ec-core` (the event-contract half, built on this SDK) has no operator mode at all. Contrast `signRedeemAuth`/`redeemFor`, which gives claims exactly this shape for binaries (relayer submits, payout hard-pinned to the owner). Placement is missing the equivalent. Consequence: a consumer app cannot offer a hot-key "roll bot" for Event Contracts without dropping beneath the SDK to raw contract calls — which reintroduces exactly the tick/lot/revert handling the SDK exists to own. Either a `placeOrderFor`-aware binary trader path or a Bot Kit `ec-core` operator mode would close it.
