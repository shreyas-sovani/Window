...
## Current State
Working. Duel proofs require exact market-owned transactions; mixed or incomplete rows refuse, accepting identity comes from the completed proof URL, settlement comes from the Finalized market, and no public-tape chronology is treated as intent. The challenger fill must be the transaction the link names (`wrong-fill`), the accept floor is tape-derived (a URL `minStake` can only tighten it), and a pending proof read renders verifying, never a refusal. Up/Down Call and Exit pricing use the correct complementary side; missing liquidity and unsafe stake inputs fail closed. Fill confirmation distinguishes unavailable reads from confirmed no-fill. Auto-selection and the `best` badge score only `SELECTABLE_CADENCES` (5m–24h) — never the chipless 60s venue. Covered by the 404-test suite with fake two-wallet, replay, adapter-contract, and UI integration paths.

## Decision Log

### 2026-09-09 — Soften the accept floor by the SDK-cushion + MINT_A_PAIR tolerance so a comparable accept is not refused
- **Change**: `duel.ts` — new `ACCEPT_FLOOR_TOLERANCE = 0.4` constant; `acceptFloor(challenge)` now returns `max(minStake, stake) × ACCEPT_FLOOR_TOLERANCE` instead of the raw max. `verifyAccept`, `readDuel`, and the UI `Duel` strip / `App` accept gate all read this softened number automatically — the invariant "URL floor can only tighten" is preserved because the tolerance scales both `minStake` and `stake` uniformly. Tests updated: `duel.test.ts` acceptFloor + verifyAccept (bumped the "URL floor tightens above the tape default" case's undershoot from 12 to 6 to still fall below the softened raised floor), `Duel.test.tsx` floor display (3.96 not 6.44), `App.integration.test.tsx` floor-gate + prefill.
- **Reasoning**: With `slippageBps: 3000n` (30%) at the SDK layer (see `src/exchange/AGENTS.md` 2026-09-09), each side's tape escrow lands well below the approved stake, AND when a taker crosses a resting order on the OPPOSITE outcome (BUY_YES × BUY_NO = MINT_A_PAIR) the fill prices at `contracts − sold-leg-proceeds` — a fraction of the collateral that was locked. Live repro: opponent's FOK tx `0x70f6…a064` succeeded, locked 9.999342 tUSDC, refunded 4.859118, tape escrow landed at 5.14; challenger's tape escrow was ~8 (direct-ish fill), so the old 0.65 floor of ~5.2 refused the accept by a hair. 0.4 covers the observed MINT_A_PAIR asymmetry (5.14 / 8 = 0.64) and still refuses an accepter who committed less than 40% of the challenger's tape stake.
- **Rejected alternative(s)**: Track the challenger's approved (intent) stake through the receipt/URL and floor against that (bigger surface — new field on `CallReceipt`, `ChallengeLinkPayload`, `verifyChallenge`; the tolerance folds through the existing tape-derived floor with no schema change — this is the right long-term fix, noted in the constant's JSDoc); shrink the cushion (defeats the point of round-2 — thin books need drift tolerance); skip the floor check for FOK only (leaves `readDuel` proof reconstruction disagreeing with the accept gate); percentage-based `verifyAccept` on top of the raw floor (two sources of one invariant — one central tolerance is auditable).
- **Task/session**: Real-Shannon Call end-to-end fix (round 3, tightened in round 4 after opponent's MINT_A_PAIR-priced FOK still fell below 0.65 × challenger tape).

### 2026-09-09 — Trust the SDK's cushion in the quoted path; keep app cushion for the raw fallback
- **Change**: `call-session.ts` — `prepareQuotedCall` no longer layers `cushionedBuyLimit` on top of `input.quote.limitPrice`; it passes the SDK's protective limit + `priceRaw` through unchanged (the JSDoc on `CROSSING_CUSHION` now names this split). `prepareCall` and `prepareExit` keep the 3% app cushion because the raw-book path has no SDK cushion of its own. Tests updated to match: `prepareQuotedCall > "uses the quoted quantity and limit"` now expects the SDK's `0.4` verbatim; a new `"protective limit cushion > the SDK-quoted path passes the SDK's cushioned limit through unchanged"` case pins the invariant; the "buy's limit sits above the seen ask" case now exercises `prepareCall`; the sell-cushion case reads `0.4 × 0.97`; the FOK-accept price reads `0.5 × 1.03` (buying NO, so the cushion multiplies up).
- **Reasoning**: Live evidence — a signed BTC-15m Call reverted with `ImmediateOrCancelNoFill` on real Shannon. The App uses `quoteBinaryStake` for both solo Calls and duel accepts. The SDK already pads that quote's `limitPrice` and re-fits `quantity` so `size × limit ≤ stake` (the exact-stake approval invariant). Layering another 3% kept the SDK's quantity but raised the pool's escrow (`size × new_limit`) above the wallet's approved allowance — the tx either failed with `ERC20InsufficientAllowance` or, when close but not over, still crossed uncomfortably. The right lever is the SDK's own `slippageBps` (see `src/exchange/AGENTS.md`) — its re-fit keeps the allowance invariant.
- **Rejected alternative(s)**: Refit quantity in `prepareQuotedCall` after adding a cushion (works, but duplicates SDK logic and drops payout twice); bump `approveAmount` above the stake (surprises the user in MetaMask and violates the "approve exactly what may be spent" rule from W-031); ignore the drift entirely (the raw-book fallback still needs cushion — it has no SDK guardrail).
- **Task/session**: Real-Shannon Call end-to-end fix.

### 2026-09-08 — Chain-witness receipt: the tx receipt IS the fill proof
- **Change**: New `receipt-fill.ts` — `fillFromReceiptLogs(logs, {account, pool, collateral, yesId, noId, decimals})` + `RECEIPT_LOG_TOPICS`: ERC-20 transfers give net collateral (paid − refunds, mint-a-pair-proof), pool ERC-6901 TransferSingle/Batch give side (yesId/noId) and contracts; refuses both-sides, exits, no-leg, and failed receipts. 7 tests from live tx shapes.
- **Reasoning**: The indexer trailed chain head by 1h (then ~5m) across three live stuck-Call repros; every prior verification path (portfolio, pool tape) sits behind it. The transaction receipt is chain data the adapter already has seconds after the write — decoding its transfer logs makes verification indexer-independent and instant.
- **Rejected alternative(s)**: Decoding `OrderFilled` (order ids only — no wallet/side without more reads); balance-delta polling (loses price); extending indexer waits (the indexer is the failure).
- **Task/session**: Live-deploy fill-verification bug 4 — BACKLOG W-111.

### 2026-09-07 — MINT_A_PAIR fills price at real net cost
- **Change**: `duel.ts` — new `fillEscrow({quantity, kind, quote?, price?})`: a `MINT_A_PAIR` row's escrow is contracts − sold-leg proceeds (live row: 25.024 NO, sold YES at 0.015 for 0.375 → 24.6486 net, matching the explorer's 24.998976 − 0.350336); `tapeDuelFill` sums per-row `fillEscrow` instead of raw `quote`. `replay.ts` — `ReplayRow.kind?` and `leg()` sums `fillEscrow` (price-only rows derive proceeds as qty×price). Tests pin single, mixed, and replay cases.
- **Reasoning**: Direct indexer inspection of the stuck Call (`0xa2c8…1999`) showed `kind: MINT_A_PAIR`, `fillPrice: 0.015`, `quoteQuantity: 0.375` — the indexer reports the SOLD leg's price on mint-a-pair fills, so the old `Σ quote` escrow would have displayed a 25-contract Call as 0.37 tUSDC at 1.5% odds. Every stake/odds number (duel rows, floor, replay, receipts) flows through the one helper now.
- **Rejected alternative(s)**: Trusting `quoteQuantity` (the sold leg, not the cost); filtering mint-pair rows out of proofs (they are real fills — the winner check needs them); correcting only the receipt path (duel/replay would still lie).
...
- **Task/session**: Adversarial-review implementation pass 2 — BACKLOG W-104.

### 2026-09-06 — Proof-read honesty and tamper hardening (adversarial-review pass)
- **Change**: `duel.ts` — new `duelReadPending({hint, marketLoading, window, tapeLoading})` (a pending read is never a refusal); `verifyChallenge` refuses `wrong-fill` when the verified fill is not the hint's named tx (case-insensitive); `verifyAccept` floors a v2 accept at `max(minStake, challenge.stake)` — the URL floor can only tighten, never lower; legacy v1 links stay floorless. `series.ts` — new `SELECTABLE_CADENCES` (300/900/3600/14400/86400) as the single chip/auto-selection source. `auto-series.ts` — `seriesScore` returns -1 below 5m so `autoSeries`/`hottestCadence` never retarget into the 60s venue the nav cannot show.
- **Reasoning**: Adversarial review found the UI flashing `unknown-market`/`missing-accept-fill` refusals while proof reads were still loading, the floor trusting a URL field a tamperer could lower to admit undersized accepts, the challenger-fill↔named-tx binding living only at the App call site, and auto-jump able to select a 60s series with no chip (stranded board).
- **Rejected alternative(s)**: Rendering the refusal with a "loading…" suffix (still asserts missing evidence); trusting URL `minStake` alone; adding a 1m chip (a 1m Window is mostly lock headroom — excluded rather than selectable); flooring v1 links too (legacy semantics: anyone may accept at any stake).
- **Task/session**: Adversarial-review implementation pass — BACKLOG W-096–W-099 (duel invariants + auto-series floor, W-097).

### 2026-09-06 — Invite TTL on minted challenges
- **Change**: `challenge-link.ts` — `until?` on the v2 payload; `inviteUntil` is fill (or now) + `headroomSec`, capped at Window expiry; `challengePayloadFromReceipt` mints `until` and returns null after it; v1 envelopes carrying `until` fail closed. `duel.ts` — `ChallengeHint`/`VerifiedChallenge` carry `until`/`fillTs`; `verifyAccept` refuses `invite-expired` when the accept fill is after the effective close (hint `until` capped by fill+headroom so a tampered-late until cannot extend the invite); `readDuel` expires an unaccepted challenge with `cause: "invite" | "window"`.
- **Reasoning**: A 5m group-chat link looks live for the whole Window if it only dies at lock. Headroom is already the Call-close slice; reusing it keeps one clock. Legacy links without `until` still last until Window expiry so old URLs do not suddenly die.
- **Rejected alternative(s)**: Trusting URL `until` alone (upward tamper); always deriving TTL even without `until` (would rewrite every legacy invite); a fixed 30s for every cadence (kills 15m/1h mid-share).
- **Task/session**: Finish what the deepening pass did not ship — invite TTL.

### 2026-09-03 — Deepen the Duel: named opponents, stake floors, FOK accepts, v2 payloads, rematch prompt
- **Change**: `duel.ts` — `ChallengeHint`/`VerifiedChallenge` gain `to?`/`minStake?`; `verifyAccept` refuses `not-your-duel` (a fill not owned by the named opponent, live and on completed `&a=` proofs alike) and `below-floor` (accept escrow under the floor, 1e-6 tolerance); refusal union + copy extended. `challenge-link.ts` — payload gains `to?`/`minStake?`/`series?`; `encodeChallenge` emits the versioned **v2** envelope, `decodeChallengeLink` accepts v1 (legacy shape only — v2 fields smuggled into v1 fail closed) and v2, else null; `challengePayloadFromReceipt` mints `minStake` = challenger stake and `series` from the receipt. `call-session.ts` — `executeFokCall` + `CallWriter.fokBuy` (unified `timeInForce: "FOK"`), gated on on-chain Trading like every write. `roll.ts` — `rematchPrompt` + `RematchTarget`: the opponent-targeted successor rematch banner (kept sides, addressed link) that replaces the solo roll while armed.
- **Reasoning**: Group-chat invites leaked (anyone with the link could accept), a partial IOC could undershoot into a "duel", a dead opposite book could still mint a link, and the solo roll repeated one wallet's side instead of re-challenging the opponent. All four fixes stay on the same physics: URL remains a locator, tape + Finalized market remain truth — `to` and `minStake` are enforced against the verified fill, never trusted from the URL. FOK exists on the unified `createOrder` path (ADR-0002 clean), so the accept takes whole-or-nothing and the floor catches price-improved undershoots.
- **Rejected alternative(s)**: Dropping to `trader.placeOrder` for FOK (spec forbids; unnecessary — unified TIF exposes it); trusting `to` from the link alone (chain wins); flipping sides on rematch (kept sides are the one tested rule and preserve the opposite relationship); version-gating v1 to carry new fields (fail closed instead).
- **Task/session**: Deepen-the-Duel pass — items A1/A3/A4-domain/B/FOK.
