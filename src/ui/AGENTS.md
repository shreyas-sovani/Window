...
## Current State
Winner-pass UI plus the 2026-09-03/06 duel deepening and the 2026-09-08 demo-mode completion. Demo mode (`#/app?demo=1`) runs the same terminal on the demo adapter with a Demo badge, the demo ladder as its odds hook, an invite countdown on the challenge strip, self-contained links (`&demo=1&s=…` after the `d=` hint), and one demo-only control — the simulated opponent's accept — so the full loop records in one browser. `#/` leads with the group-chat problem and two-fill proof visual (brand "Window Duel"); its verification CTA opens the `#/docs` replay directly. `#/docs` contains current product truth (addressed invites, floor, FOK accept, invite TTL, rematch, Claim). `#/app` keeps the Window, ticket, one guided action, challenge/result, and Claim above one collapsed More drawer. Incoming challenges own the only accept CTA and render a verifying state (`DuelVerifying`) while their chain reads are in flight — never a refusal flash. On a floored challenge the stake prefills to the enforced floor (`acceptFloor`) and the accept disables with its reason below it — no below-floor fill is ever sent. The challenge strip styles the optional opponent field inside the dashed card and shares via the native sheet with a clipboard fallback (`share.ts`). Duel fill rows are labeled Challenger/Acceptor/Winner/Loser. A verified accept updates the URL with its exact tx and shows **Share verified duel**; without that tx, unrelated book activity cannot open the Duel. Odds use the recycle-safe live hook with a polled fallback. Wallet writes remain serialized, receipts require tape verification, proof-read errors are visible, and responsive/reduced-motion behavior is covered by the existing visual system. Scroll reveals fail open when IntersectionObserver is unavailable.

## Decision Log

### 2026-09-09 — Claim: invalidate on settle, invalidate everything after redeem
- **Change**: `App.tsx` — new effect fires `qc.invalidateQueries({ queryKey: ["claims", address] })` the moment `duel.kind` becomes `settled | void` or the live Window Finalizes (`status === 4`). `claimAll` now invalidates `["claims", address]`, `["fills", address]`, `["pnl", address]`, `["duelfills"]`, `["duelstatus"]` — the panels sitting above the Claim button all read outcome balances or wallet history that the redeem just changed.
- **Reasoning**: Live report — winner reaches the result screen but the Claim tote appears late (up to 60s: the `claimsQ` refetch interval), and after the redeem the panel doesn't clear because the stale preview keeps `claims.windows > 0`. Effect + invalidate delivers the tote within seconds of settlement (paired with the parallel-20 scan in `src/exchange/AGENTS.md` 2026-09-09), and the wider invalidate after `claimAll` makes duel + P&L reflect the payout the same second the toast lands.
- **Rejected alternative(s)**: Drop `claimsQ` refetch interval to 6s globally (60× the indexer load for a rare event); reset the query state manually (invalidate is the same primitive without cache-versioning traps); block on `qc.refetchQueries` (holds the write mutex on a read the user is not waiting for).
- **Task/session**: Real-Shannon Claim end-to-end fix.

### 2026-09-09 — callSide re-quotes at click time so the wallet signature always carries a fresh limit
- **Change**: `App.tsx callSide` now calls `exchange.quoteStake(win.marketId, side, quoteStakeRaw)` once inside the write guard (before the IOC/FOK write) and rebuilds the intent through `prepareQuotedCall`. If the fresh quote fails or returns null, the previously cached intent still runs — the click is never silently dropped.
- **Reasoning**: The polled `upQuoteQ`/`downQuoteQ` refresh every 4s (see `refetchInterval: 4_000`), and MetaMask signing adds several more, so the limit sent on-chain can trail the book by 5-10s. A live BTC-15m Call still reverted `ImmediateOrCancelNoFill` (`0x659b…291d` — decoded params yesPrice 0.348 / qty 15.336 for BUY_NO) even after the SDK cushion was widened, because Shannon's binary book moves through the cushion in that window. Re-quoting at click time collapses the stale window to a single round-trip through `quoteBinaryStake`, and pairs cleanly with the widened `slippageBps: 3000n` (see `src/exchange/AGENTS.md` 2026-09-09) — fresh + wider cushion is the belt-and-braces the thin book needs.
- **Rejected alternative(s)**: Shorten `refetchInterval` to 1s (kills the indexer with quote traffic on every mounted terminal); block the write on a `queryClient.invalidateQueries` + wait (adds React round-trips + timing races to a mutex-held path); refuse if the fresh quote diverges from the cached one (drops legitimate clicks when the book just quiesced — the goal is to succeed, not to explain).
- **Task/session**: Real-Shannon Call end-to-end fix (round 2).

### 2026-09-08 — Demo links that actually resolve, and three gate bugs the recording path hit
- **Change**: `App.tsx` — `demoDecorateHref` now prepends the href it decorates (it was minting `&demo=1&s=…` with the route and the `d=` hint dropped, so every demo challenge link was dead); `duelWrongViewer` requires a connected wallet (an addressed link left its recipient with a disabled "Open this link with 0x…" and, because `onlySide` hides the onboarding panel, no way to connect at all); the accept floor gate reads the accept side's **quoted escrow** instead of the typed stake and the prefill lands a cent clear of the floor (lot/tick quantization escrowed a hair under the floor, so a legitimate accept was refused after the FOK filled); `completeFill` invalidates `dueltape`/`duelstatus` and records `acceptPublished`, and a published-but-not-yet-tape-read accept renders `DuelVerifying` instead of "the accepting transaction could not be verified"; new demo-only `demoOpponentAccept` (simulated opponent takes the other side through `takeAs`, verified on the demo tape before the `&a=` URL publishes). `ChallengeStrip.tsx` — the strip prints `Invite closes in m:ss` from the payload's `until`, and takes an `extra` slot (the demo-opponent control). `router.ts` — new `bootstrapSearchLink(search, hash)` lifts a fragment-stripped link's query into `#/app?…`. `CallBoard.tsx` — `Busy` gains `demo-accept`. New `App.demo.test.tsx` (4 tests): badge + Strong ladder, gate walk to a verified Call and a self-contained link, one-browser duel, and the opponent tab verifying from the blob alone.
- **Reasoning**: The demo link was the product's whole distribution claim and it did not contain a challenge. The other three were latent in live mode too — all three only bite on the path a recipient walks, which is exactly the path no test covered until now.
- **Rejected alternative(s)**: Waiting out the 8s duel-tape poll after an accept (shows a refusal we know is false); clamping the stake input to the floor continuously (fights the user — the gate explains instead); keeping the addressed-wallet copy for disconnected viewers (honest but a dead end); a timed demo opponent (accepts before the link is shared).
- **Task/session**: Demo-mode completion pass — BACKLOG W-117/W-118.
...
- **Rejected alternative(s)**: Refusal-with-loading-suffix (dishonest); keeping Copy beside Share (two controls, one job); fall-through-to-clipboard on share dismissal (surprise clipboard write when the user cancelled).
- **Task/session**: Adversarial-review implementation pass — BACKLOG W-096/W-101/W-102 UI side.

### 2026-09-06 — Opponent chrome, invite-close copy, Landing/Docs truth
- **Change**: `ChallengeStrip` accepts children; the opponent input lives inside `.challenge-strip`. `styles.css` adds `.challenge-to` / `.duel-floor`. `Duel.tsx` shows invite-close on a live challenge and distinct expired copy for `cause: "invite"` vs Window lock. `Landing.tsx` / `Docs.tsx` name addressed invites, the stake floor, FOK accepts, invite TTL, rematch, and Claim on the result.
- **Reasoning**: The opponent field shipped unstyled and outside the strip, so the addressed-invite control looked like leftover form chrome. Landing/Docs still described an open-ended IOC challenge.
- **Rejected alternative(s)**: Forcing a named opponent on every mint (group-chat open invites stay valid); a second card for the address field.
- **Task/session**: Finish what the deepening pass did not ship.

### 2026-09-03 — Deepen the Duel UI: addressed strips, floor hints, gated accepts, rematch + claim
- **Change**: `ChallengeStrip.tsx` — the gate takes `allow` (no link while the opposite side of the live Window has no executable depth) and `to` (pre-named opponent for rematches); the strip grows an "Opponent wallet" input that addresses the link (`to`), invalid input ignored. `Duel.tsx` — challenge stage shows "Stake at least N tUSDC" when `minStake` is set; settled/void stages gain Claim (owed participant only — winner, or either on void) and "Rematch {opponent} — Call {side} on the next Window" buttons. `App.tsx` — `duelWrongViewer` disables accept for anyone who is not `to` (label names the wallet); accept disabled when the acceptor side has no executable plan; the accept `callSide(duelAcceptSide, "fok")` uses FOK; `duelRematch` finds the successor Window and arms `rematchTarget`, the rematch banner (`rematchPrompt`) replaces the solo roll while armed and the strip then mints the addressed link; `duelClaim` wires Claim off the result via the existing session.
- **Reasoning**: The leaks were product-shaped, not code-shaped — anyone could accept a pasted link, undershoots could pose as accepts, dead books could mint expiring links, and Rematch ignored the opponent. Every fix renders from verified fills: the addressed wallet and the floor are enforced by `verifyAccept` against the tape, the UI only communicates them.
- **Rejected alternative(s)**: Trusting the URL's `to` for gating alone (chain wins, always); hiding the floor (silent refusals are worse than a stated minimum); a rematch that flips sides (untested rule, breaks the opposite-side invariant).
- **Task/session**: Deepen-the-Duel pass — items A1–A5 UI.
...
## Known Gotchas
`onlySide` (an incoming challenge) hides the onboarding panel and the ticket-side approve — the duel CTA is then the only control, so every wallet step must remain reachable through it. `demoDecorateHref` returns the full href; never let it return only the suffix. The floor gate must compare escrow, not the typed stake. Each onchain button has its own busy flag. Approve uses two-phase pending (hash wait + 4s cooldown). Never import `.env` keys here. Do not pin venue from the first BTC row — 15m+ live on a second venue.
