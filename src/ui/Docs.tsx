...
const SECTIONS: { id: string; title: string; body: { h?: string; p?: string }[] }[] = [
  ...
  },
  {
    ...
    body: [
      { h: "The loop" },
      { p: "Challenge another wallet on the same Window. Two opposite Calls, two verified fills, one Line, one on-chain winner. A minted link can name its opponent and floors the accept at the challenger's stake. Afterward, a participant can rematch the same opponent on the successor Window (kept sides) or Claim from the result." },
      { h: "Opponents, not counterparties" },
      { p: "A duel is social. Each wallet sends its own take (IOC for a solo Call, FOK for a duel accept); the wallets never fill against each other, no pot is matched, and one invite does not promise the second fill. A duel does not exist until two fills on the same marketId are verified on-chain — a signed tx that did not fill, or a URL field, is never a success state." },
      { h: "Judging" },
      { p: "The challenge link (#/app?d=…) is only a locator: the terminal re-verifies the challenger fill. After the accepting Call verifies, Window appends its exact transaction as &a=… and exposes a completed proof link. Unrelated opposite fills on the public book never become an opponent. An addressed link (`to`) only accepts that wallet's fill; a below-floor fill is not an accept; the invite closes after the Window's Call headroom from the challenger fill (or sooner if the Window locks). The winner is the wallet whose filled side matches settlement; unequal stakes at or above the floor are allowed and shown; a Void is a draw; one fill after expiry or a closed invite is not a win; the same wallet cannot accept its own challenge." },
      { h: "Replay" },
