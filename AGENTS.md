...
- **Adversarial-review hardening pass (2026-09-06, this session):** external review findings implemented as W-096–W-103 — challenge links render a **verifying** state while their market/tape reads load (no refusal flash; `duelReadPending` + `DuelVerifying`); the accept floor is **tape-derived** (`max(URL minStake, challenger tape escrow)` — tampering the URL floor down admits nothing; v1 stays floorless); `verifyChallenge` refuses `wrong-fill` (the fill must be the named tx); `fillsByPool` pages the indexer tape (`readTapePages`, cap 2000) so deep named txs verify; `SELECTABLE_CADENCES` is the single chip/auto-selection source and autoSeries/`best` never retarget into the chipless 60s venue; duel rows labeled Challenger/Acceptor/Winner/Loser; `share.ts` unifies Web Share → clipboard on the strip and receipts; brand unified to **Window Duel** (Landing masthead, README title); new `docs/VERIFICATION.md` evidence file. 378 tests green, build green. Deploy is done by the human (live on Vercel); W-083 remainder (real Shannon proof tuple + recording + one DEMO walkthrough) stays human-blocked.
