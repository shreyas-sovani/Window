...
- Domain modules, Vite app, Shannon ExchangeAdapter, fake adapter, Call session, Claim session, Claim primary, Window board, Stake quote sizing, Book drawer, Rest quote (post-only, not on Call), Series record, Series P&L, Board notice, Settle preview, venue settlement fee, Wallet P&L, demo-hardening UX, Window phase, cadence labels, bounded stake allowance, and Explorer proof toasts are in. UI is now the light "paper ledger" theme (oat/ink/clay, Lora + Schibsted + Plex Mono — see src/ui/AGENTS.md). Vite already splits vendor/chain/markets chunks. Next: live-browser visual QA or a deployed demo URL.
