...
- Domain modules, Vite app, Shannon ExchangeAdapter, fake adapter, Call session, Claim session (`readClaimSession` Windows + expected tUSDC; `executeClaims` continues after a failed Window), Claim primary, Window board, Stake quote sizing, Book drawer, Rest quote (post-only, not on Call), Series record, Series P&L, Board notice, Crash notice (Retry, no stack), Settle preview, venue settlement fee, Wallet P&L, demo-hardening UX, Window phase, cadence labels, tote clock hours on 1h+ Windows, bounded stake allowance, Explorer proof toasts (Call/Exit/Rest/cancel/approve/Claim/faucet; `shorten` does not double short hashes), series-history oracle receipts, Line on history chips, Pulse (`pulseReady`, marketId-keyed tape, paper-ledger CSS), hash routes (`parseRoute` exact path), and RevertCopy for Call-path throws (`below-lot`, not Trading, SignerRequired) are in. Three-page site: `#/` landing, `#/docs` docs, `#/app` terminal. UI is the light "paper ledger" theme (oat/ink/clay, Lora + Schibsted Grotesk + IBM Plex Mono — see src/ui/AGENTS.md). Vite already splits vendor/chain/markets chunks. Next: live-browser visual QA or a deployed demo URL.
